...
def generateJSON(numTargets, movementCycle, cycleDistribution, rotationAngle, SPEternary, targetDistance, numDemoCycles, demoTargetAngle, filenum):
...
for iter in range(1, 101):
    ...
    # pseudorandomly generate SPE ternary that determines the presence and direction of the error clamp
    #       don't allow more than 2 trials with the same error presentation
    #       could probably be more clever with this and limit the random  number generation for trials with the rotation on
    SPEternary = []
    oneback = np.nan
    twoback = np.nan
    for i in range(0, 115):  # 2nd number should be number of trials in the experiment
        tempval = random.randint(-1, 1)
        while ((oneback == twoback) and (tempval == oneback)):
            tempval = random.randint(-1, 1)
        SPEternary.append(tempval)
        twoback = oneback
        oneback = tempval
